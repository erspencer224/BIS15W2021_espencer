...
Why do we have NA here? Do all of these penguins lack data? **If there is one NA, NA will be spit out**
```{r}
penguins %>% 
  group_by(island) %>% 
...
`count()` is an easy way of determining how many observations you have within a column. It acts like a combination of `group_by()` and `n()`. **Count is a combination of summarize and group_by**
```{r}
penguins %>% 
  count(island, sort = TRUE) #sort=TRUE sorts the column in descending order
```
...
```{r}
penguins %>% 
  count(island, species, sort = F) %>% #F means you aren't sorting in any way
  arrange(n) #makes it into ascending order 
```
