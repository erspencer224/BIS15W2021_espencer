...
## 1              203
```
**There are 203 different countries represented in the data.**
...
## 1             1551
```
**There were 1,551 distinct fish species caught as part of these data.**
...
## # ... with 183 more rows
```
**China had the overall largest catch in the year 2000.**
...
fisheries_tidy_refocused %>% 
  ...
  group_by(country) %>%
  summarize(sum_sardines=sum(catch, na.rm=T)) %>% 
  arrange(desc(sum_sardines))
```

```
## # A tibble: 37 x 2
##    country               sum_sardines
##    <fct>                        <dbl>
##  1 Morocco                       7470
##  2 Spain                         3507
...
## 5 Chile                         2775
```
**China, Republic of Korea, Peru, Japan, and Chile caught the most cephalopods between 2008 and 2012**
...
```
## # A tibble: 1,471 x 2
##    asfis_species_name    largest_catch
##    <chr>                         <dbl>
...
```
**The species with the highest total catch between 2008 and 2012 is the Theragra chalcogramma**

10. Use the data to do at least one analysis of your choice.

```r
fisheries_tidy %>% 
  filter(str_detect(common_name, "Catsharks")) %>% 
  group_by(country) %>% 
  summarize(sum_catsharks=sum(catch, na.rm=T)) %>% 
  arrange(desc(sum_catsharks))
```

```
## # A tibble: 12 x 2
##    country               sum_catsharks
##    <fct>                         <dbl>
##  1 Tunisia                         590
##  2 France                          381
##  3 Spain                           282
##  4 United Kingdom                  158
##  5 Algeria                          49
##  6 Croatia                          48
##  7 Montenegro                       41
##  8 Portugal                         11
...
## 10 Albania                           7
## 11 Netherlands                       6
## 12 Serbia and Montenegro             6
```
**For this question, I wanted to see which country caught the largest amount of Catsharks throughout the data. Turns out it is Tunisia, a country in North Africa.**
