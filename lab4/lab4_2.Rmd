...
```
**Select is for columns, filter is for looking within rows**
## `filter()`
...
```
Fish is the dataframe, lakeid is being restricted to AL
...
Using the `!` operator allows for the exclusion of specific observations. ! = not operator
```{r}
filter(fish, lakeid != "AL")
```
^everything that is NOT AL
...
```
Filtering fish, inside of the length column for length 167 and 175 and nothing else (2 portential targets)
...
```
Filtering between 2.5 and 2.55 for scalelength 

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2))
```
Values above and below 2 that are within .2 for the radii length
...
view(x)
```
**This is a way to check that your code is working properly**
...
```
For the above, both of these conditions must be met in order for the data point to appear (and)

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length >= 350)
```
For the above, either one of these conditions must be met in order for the data point to appear (or) 
...
bovidae
```
"Bovidae" identifies the string you are searching for 
...
glimpse(mammals)
```
The dataset is using -999 instead of NA, which will mess up the data 
