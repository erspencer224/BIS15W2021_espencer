...
# If there is any missing data within a list, things such as mean cannot be calculated from it. This means you don't want to have missing data included. You could not include the NA into the vector when calculating the mean but then also make a note saying that you have missing data and therefore this isn't technically the actual mean.

4. Recalculate the mean using the following code chunk. Why is the useful?  
# this is useful because it allows you to calculate the mean even if you don't have all of your data.
```{r}
