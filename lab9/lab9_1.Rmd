...
```{r}
naniar::miss_var_summary(homerange) # to find where NAs are present and how many there are
```
...
# could also just do ggplot(homerange, aes(x=log etc))
```
...
homerange %>% 
  ...
  geom_col()
```
# use geom_bar for counts, and geom_col for specifying an x and y axis
