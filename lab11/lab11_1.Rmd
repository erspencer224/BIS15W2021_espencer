...
```{r}
deserts$year <- as.factor(deserts$year) #needed to be changed to a factor
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.6, color = "black", fill = "deeppink4", bins=40)+ #alpha refers to transparancy 
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = ..density..), binwidth = .4, fill = "deepskyblue4", alpha = 0.6, color = "black")+
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=..density..), bindwidth=0.4, fill="deepskyblue4", alpha=0.6, color="black")+
  geom_density(color="red")+
  labs(title="Distribution of Homerange")
```
