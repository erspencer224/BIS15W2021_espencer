...
+`scale_colour_brewer()` is for points (for geom_point)
+`scale_fill_brewer()` is for fills (for geom_bar)
...
```{r}
p2+facet_grid(thermoregulation~class) #use grid for 2 categorical variables ie: termoregulation and class
```
Exporting to a PDF ??????????
```{r}
pdf("homerange.pdf")
