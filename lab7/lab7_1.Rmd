...
Keep in mind, if the authors of the data represented NA's in another way, they will not be detected by R. This one is sort of a mess to use.  
```{r}
is.na(msleep)
```

We also learned... But this one doesn't tell you where the NA's are 
```{r}
any_na(msleep)
```
...
amniota %>% 
  summarise_all(~(sum(is.na(.))))
```
~ means you don't know where all of the na's are in the data ? 
